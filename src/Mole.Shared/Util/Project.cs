    ...
    public class Project
    {
        ...
        public Project(Progress progress, string dir)
        {
            ...
            new Thread(() => {
                ...
                if (!CheckAllHashes())
                    throw new Exception("Hash mismatch found!");
        ...
        public bool CheckAllHashes()
        {
            if (!CheckHash(RomPath)) {
                LoggerEntry.Logger.Error($"Hash mismatch: {RomPath}");
                return false;
            }
            foreach (var p in StackPaths) 
                if (!CheckHash(p)) {
                    LoggerEntry.Logger.Error($"Hash mismatch: {p}");
                    return false;
                }
        ...
        public static bool CheckHash(string path)
            => Hash.Sha1(path) == Hash.Sha1($"{path}.sha1");
    }
}